...
public abstract class HVC {
    ...
    protected int GetVersion(int inTimeOutTime, byte[] outStatus, HVC_VER ver) {
        ...
        ver.rev = (int)(recvData[ver.str.length + 3] & 0xff) +
                  (int)((recvData[ver.str.length + 4] & 0xff) << 8) +
                  (int)((recvData[ver.str.length + 5] & 0xff) << 16) +
                  (int)((recvData[ver.str.length + 6] & 0xff) << 24);
        return nRet;
    }
    ...
    protected int GetThreshold(int inTimeOutTime, byte[] outStatus, HVC_PRM param) {
        ...
        /* Receive data */
        nRet = ReceiveData(inTimeOutTime, nSize[0], recvData);
        param.body.Threshold = ((int)recvData[0] & 0xff) + (recvData[1] << 8);
        param.hand.Threshold = ((int)recvData[2] & 0xff) + (recvData[3] << 8);
        param.face.Threshold = ((int)recvData[4] & 0xff) + (recvData[5] << 8);
        return nRet;
    }
    ...
    protected int GetSizeRange(int inTimeOutTime, byte[] outStatus, HVC_PRM param) {
        ...
        /* Receive data */
        nRet = ReceiveData(inTimeOutTime, nSize[0], recvData);
        param.body.MinSize = ((int)recvData[0] & 0xff) + (recvData[1] << 8);
        param.body.MaxSize = ((int)recvData[2] & 0xff) + (recvData[3] << 8);
        param.hand.MinSize = ((int)recvData[4] & 0xff) + (recvData[5] << 8);
        param.hand.MaxSize = ((int)recvData[6] & 0xff) + (recvData[7] << 8);
        param.face.MinSize = ((int)recvData[8] & 0xff) + (recvData[9] << 8);
        param.face.MaxSize = ((int)recvData[10] & 0xff) + (recvData[11] << 8);
        return nRet;
    }
